...
class TestComponent {
    ...
    public methodObservableWithPipeSubscription(): Subscription {
        return new BehaviorSubject(0)
            .asObservable()
            .pipe(switchMap(() => new BehaviorSubject(123).pipe()))
            .subscribe();
    }
}
...
describe('AutoUnsubscribe', () => {
    ...
    it('should unsubscribe from observables with pipe after ngOnDestroy called', () => {
        const subscription = component.parameter$.pipe(switchMap(() => of(1))).subscribe();
    ...
    it('should unsubscribe from observables with lift after ngOnDestroy called', () => {
        const subscription = component.parameter$.lift(switchMap(() => of(1))).subscribe();
